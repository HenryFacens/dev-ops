pipeline {
    ...
    stages {
        ...
        stage('Image_Docker - Build & Push') {
            environment { IMAGE_TAG = "${env.BUILD_NUMBER}" }
            steps {
    ...
    post {
        success { echo "Pipeline concluído com sucesso. Build ${env.BUILD_NUMBER}" }
        failure { echo "Pipeline falhou. Verificar estágios anteriores." }
